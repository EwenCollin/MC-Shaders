...
void main() {
    ...
    if(Position.x + ChunkOffset.x > distanceToAA) {
        gl_Position = ProjMat * ModelViewMat * vec4( (- Position.y - ChunkOffset.y + distanceToAA)*cos(angle) + (Position.x + ChunkOffset.x)*sin(angle), (Position.x + ChunkOffset.x - distanceToAA)*cos(angle) + (Position.y + ChunkOffset.y)*sin(angle), Position.z + ChunkOffset.z, 1.0);
    }

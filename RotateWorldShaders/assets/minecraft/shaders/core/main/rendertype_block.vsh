...
void main() {
    ...
    float angle = -GameTime*100;
    float mod_angle = mod(angle, twoPi);
    if(mod_angle > pi/2) {
        angle = 0;
    }
    if (mod_angle < pi && mod_angle > pi/2) {
        angle = (pi/2);
    }
    if(animationMode == 0) angle = 0;
    vec4 angle_position = vec4( (- Position.y - ChunkOffset.y + distanceToAA - 1.8)*cos(angle) + (Position.x + ChunkOffset.x)*sin(angle), (Position.x + ChunkOffset.x - distanceToAA - 1.8)*cos(angle) + (Position.y + ChunkOffset.y)*sin(angle), Position.z + ChunkOffset.z, 1.0);
    if(Position.x + ChunkOffset.x > distanceToAA) {
        gl_Position = ProjMat * ModelViewMat * angle_position;
    }

...
void main() {
    ...
    if(Position.x + ChunkOffset.x > 20.0) {
        if (Position.x + ChunkOffset.x > 0.0) { // Positive/Negative X
            gl_Position = ProjMat * ModelViewMat * vec4( - Position.y - ChunkOffset.y + 18.0, Position.x + ChunkOffset.x - 22.0, Position.z + ChunkOffset.z, 1.0);

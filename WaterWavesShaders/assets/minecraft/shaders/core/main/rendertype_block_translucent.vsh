...
void main() {
    ...
    vec3 relative_position = Position + ChunkOffset;
    float angle = GameTime*waveTimeMultiplier;
    if(isDistanceBased == 1) {
        gl_Position += vec4(0.0, 2.0*sin(angle + (vertexDistance/waveScaleDivider)), 0.0, 0.0);
    }
    else {
        gl_Position += vec4(0.0, 2.0*sin(angle + (relative_position.x/waveScaleDivider)), 0.0, 0.0);
    }
    /*
    if(Position.x + ChunkOffset.x > distanceToAA) {
        gl_Position = ProjMat * ModelViewMat * vec4( (- Position.y - ChunkOffset.y + 18.0)*cos(angle) + (Position.x + ChunkOffset.x)*sin(angle), (Position.x + ChunkOffset.x - 20.0)*cos(angle) + (Position.y + ChunkOffset.y)*sin(angle), Position.z + ChunkOffset.z, 1.0);
    }*/
    if (fadeAmount > 20.0 * fadeScale) {

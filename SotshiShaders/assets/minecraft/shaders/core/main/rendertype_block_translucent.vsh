...
void main() {
    ...
    vec3 relative_position = (ModelViewMat * vec4(Position + ChunkOffset, 1.0)).xyz;
    // Position with offset
    if(Position.x + ChunkOffset.x > distanceToAA) {
        gl_Position = ProjMat * ModelViewMat * vec4( - Position.y - ChunkOffset.y + (distanceToAA - 1.8), Position.x + ChunkOffset.x - (distanceToAA + 1.8), Position.z + ChunkOffset.z, 1.0);
    }
    else if (Position.x + ChunkOffset.x < - distanceToAA) {
        gl_Position = ProjMat * ModelViewMat * vec4(Position.y + ChunkOffset.y - (distanceToAA - 1.8), - Position.x - ChunkOffset.x - (distanceToAA + 1.8), Position.z + ChunkOffset.z, 1.0);
    }
    else if (Position.z + ChunkOffset.z > distanceToAA) {
        gl_Position = ProjMat * ModelViewMat * vec4(Position.x + ChunkOffset.x, Position.z + ChunkOffset.z - (distanceToAA + 1.8), - Position.y - ChunkOffset.y + (distanceToAA - 1.8), 1.0);
    }
    else if (Position.z + ChunkOffset.z < - distanceToAA) {
        gl_Position = ProjMat * ModelViewMat * vec4(Position.x + ChunkOffset.x, - Position.z - ChunkOffset.z - (distanceToAA + 1.8), Position.y + ChunkOffset.y - (distanceToAA - 1.8), 1.0);
    }
